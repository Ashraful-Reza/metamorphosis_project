...
class PurchaseRequestLine(models.Model):
    ...
    # //attaachment_ids and attachment_count fields add by zahid
    attachment_ids = fields.Many2many(
        ...
        string="Attachments",
        help="Attach documents related to this purchase request line.",
    )
    ...
    expenditure = fields.Char(string="Expenditure", related="request_id.expenditure", store=True)
    ...
    # //last_purchase_price fields add by zahid
    ...
    def _compute_last_purchase_price(self):
        for line in self:
            if line.product_id:
                # Search for the last purchase.order.line based on the product
                query = """
                    ...
                    LIMIT 1
                """
                self.env.cr.execute(query, (line.product_id.id,))
                result = self.env.cr.fetchone()

                # Assign the price or fallback to 0.0
                line.last_purchase_price = result[0] if result else 0.0
            else:
    ...
    unit = fields.Many2one(comodel_name="stock.warehouse", string="Unit")
    # add start_date field in detail line by zahid
    start_date = fields.Date(
        string="Start Date",
        tracking=True,
        default=fields.Date.context_today,
    )
    # add end_date field in detail line by zahid
    end_date = fields.Date(
        string="End Date",
        tracking=True,
        default=fields.Date.context_today,
    )
    # add remarks field by zahid
    remarks = fields.Text(string="Remarks")
    # add sub_category field by zahid
    sub_category = fields.Char(
        string="Sub Category",
        compute="_compute_sub_category",
        store=True
    )

    # add for invisible start_date and end_date for product type is service by zahid
    product_detailed_type = fields.Selection(
        ...
        string="Product Type",
        compute="_compute_product_detailed_type",
        store=True,
    )

    # add for invisible start_date and end_date for product type is service by zahid
    @api.depends('product_id')
    def _compute_product_detailed_type(self):
        for line in self:
            line.product_detailed_type = line.product_id.detailed_type if line.product_id else False

    # add by zahid
    # @api.depends('product_id.categ_id.parent_id.name')
    @api.depends('product_id.categ_id.name')
    def _compute_sub_category(self):
        for record in self:
            # record.sub_category = record.product_id.categ_id.parent_id.name if record.product_id.categ_id.parent_id else ''
            record.sub_category = record.product_id.categ_id.name if record.product_id.categ_id else ''

    # //add by zahid
    @api.onchange('product_qty', 'last_purchase_price', 'unit_price')
    def onchange_estimated_cost(self):
        for line in self:
            if line.last_purchase_price > 0.00 and line.unit_price <= 0.00:
                line.estimated_cost = line.product_qty * line.last_purchase_price

    # @api.onchange('unit_price', 'product_qty', last_purchase_price)
    ...
    def onchange_product_id(self):
        if self.product_id:
            ...
            # change because product_uom_id field need uom_po_id value by zahid
